...
| MCP discovery surface — verified | New MCP clients receive the bounded nine-tool day-to-day surface through `tools/list`; the complete advanced callable catalog remains an implementation/compatibility surface and is not a default discovery claim. Earlier Glama evidence cited a successful 57-tool introspection, and an earlier source audit recorded 61; retain both as historical evidence only. Record the target Smithery build/smoke-test `tools/list` receipt before publishing any target-specific claim. | Maintainer |
