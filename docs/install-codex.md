...
The Codex MCP process must load an **immutable installed snapshot**, not an
editable source checkout. `pip install -e .` makes every repository edit change
the on-disk fingerprint while a live MCP process still has old imports.
...
From a clone, install a non-editable copy (builds a wheel, then installs it):
...
`memoryguard_provider_install` (provider: `codex`) detects editable/local-source
installs and, before writing `python -m memoryguard.mcp_server`, reuses or
atomically builds a content-keyed non-editable snapshot under the MemoryGuard
data home (`mcp-runtime/<source-key>/`). Unchanged source reuses the snapshot;
changes to packaged runtime files (Python, GUI JS/icons, and other package
data) select a new directory. Build/cache/VCS artifacts and generated bytecode
are ignored. A failed build leaves the previously selected snapshot and
provider config untouched. Diagnostics never change the user install.
Repository tests keep using `src`.

`PYTHONPATH=.../src` is a developer fallback only; do not point Codex MCP at it.
