...
SAGA_PHASES = (
    ...
    "verify_source_links", "activate_canonical", "shadow_legacy",
    "drain_outbox", "build_projection", "verify_readiness",
    "canonical_ready",
)
...
def build_bundles(
    ...
    """
    group_agent_ids = {
        str(record.agent_instance_id or "")
        for record in legacy.list_records()
        if record.agent_instance_id
    }
...
class RuleReconciliationService:
    ...
    def source_digest(self, share_group_id: str) -> str:
        ...
        always_active = _active_mandatory(legacy)
        payload: list[Any] = []
        for record in sorted(always_active, key=lambda record: record.memory_id):
            assignments = sorted(
                (a.target_type, a.target_id, a.project_ref, a.effect,
                 int(a.priority_override or 0))
    ...
    def canonical_digest(self, share_group_id: str) -> str:
        ...
        return stable_hash(
            "canonical-digest",
            json.dumps(links, ensure_ascii=False, sort_keys=True),
        )
    ...
    def _start_or_resume_job(
        ...
        *,
        ...
        # Second-build idempotency: a group already at canonical_ready is
        # confirmed read-only -- never re-written.  This keeps a re-run from
        # bumping link/record updated_at or minting a second job row.
        ...
        job_id = RuleReconciliationStore.job_id_for(share_group_id, digest)
        ...
        if job["status"] == "retryable_failed":
            return self.jobs.transition(
                job_id, status="pending_model", phase="model", last_error="",
            )
        return job
        ...
        for source_id in source_ids:
            for assignment in legacy.list_rule_assignments(source_id):
                key = (
                    ...
                    assignment.project_ref, assignment.effect,
                )
                if key in seen:
                    continue
                seen.add(key)
    ...
    def _binding_from_assignment(
        ...
        *,
        ...
        binding = build_binding(
            ...
            priority=0,
            owner_agent_id=getattr(assignment, "agent_instance_id", ""),
    ...
    def _write_canonical(
        ...
        for bundle in bundles:
            ...
            canonical_memory_id = stable_hash(
                ...
                bundle.project_ref or bundle.provider or "all",
    ...
    def _verify_source_links(self, share_group_id: str) -> list[str]:
        ...
        return [
            record.memory_id for record in _active_mandatory(legacy)
    ...
    def run(
        ...
        *,
        ...
        ``bundle_plan`` overrides the model step (used by the real-snapshot
        acceptance for determinism).  When omitted, the plan is derived
        heuristically via :func:`build_bundles`.
        """
        ...
        job_id = job["job_id"]
        # Second-build idempotency: confirm readiness read-only, never re-write.
    ...
    def _run_locked(
        ...
        *,
        ...
        job_id = job["job_id"]
        legacy = self._legacy(share_group_id)
        self.jobs.transition(job_id, status="model_running", phase="model")
        ...
        if bundle_plan is None:
            ...
            if not active:
                self.jobs.transition(
                    ...
                    last_error="no_active_mandatory",
                )
                raise ValueError("no_active_mandatory")
            plan = build_bundles(self.store, legacy, share_group_id, active)
            effective_model_mode = "heuristic"
        else:
            plan = bundle_plan
            effective_model_mode = model_mode or "scripted"
        validate_bundles(plan, [
            record.memory_id for record in _active_mandatory(legacy)
        ])
        ...
        self.jobs.transition(
            ...
            result_json=json.dumps(
                {
                    "bundles": [
                        bundle.to_dict() if isinstance(bundle, ScopeBundle)
                        else ScopeBundle.from_dict(bundle).to_dict()
        ...
        self._write_canonical(share_group_id, legacy, plan)
        ...
        # ---- activate_canonical -------------------------------------------
        self.jobs.transition(job_id, phase="activate_canonical")
        ...
        return self.jobs.transition(
            ...
            canonical_digest_after=self.canonical_digest(share_group_id),
...
def ensure_reconciliation_job(
    ...
    *,
    ...
    never been scope-bundled.  Idempotent: an existing canonical_ready job
    blocks a second job, and an existing matching job is returned unchanged.
    """
    from .shared_memory_store import SharedMemoryStore
    ...
    # Gate: canonical definitions == 0 OR source links incomplete.
    links = store.list_source_links(share_group_id=share_group_id)
    canonical_defs = {
    ...
    if source_links_complete:
        return {"created": False, "reason": "canonical_already_built"}
    # Gate: no canonical_ready job.
    latest = jobs.latest_job(share_group_id)
    if latest is not None and latest["status"] == "canonical_ready":
        return {"created": False, "reason": "canonical_ready_job_exists"}
    if latest is not None and latest["reason"] == REASON_LEGACY_HEURISTIC:
        if latest["status"] in {"pending_model", "model_running", "staged",
                                "applying", "retryable_failed"}:
            return {"created": False, "existing_job": latest,
                    "reason": "legacy_heuristic_job_already_pending"}
    # Gate: history only record enrichment, result source heuristic, no
    # scope_bundle task anywhere.
    ...
    if latest is not None:
        return {"created": False, "existing_job": latest,
                "reason": "existing_reconciliation_job"}
    source_digest = RuleReconciliationService(
        store, workspace=workspace,
    ).source_digest(share_group_id)
    job = jobs.create_job(
        share_group_id=share_group_id,
