...
_TABLES: dict[str, dict[str, TableSpec]] = {
    ...
    "system": {"schema_meta": _t("schema_meta", ("domain", "version", "marker", "updated_at"), primary_key="domain"), "manifest": _t("manifest", ("manifest_id", "state", "generation", "migration_id", "source_digest", "target_digest", "manifest_digest", "digests_json", "errors_json", "last_error", "workspace_source_pointer", "global_source_pointer", "data_home_root", "checkpoints_json", "created_at", "updated_at"), primary_key="manifest_id"), "migration_ledger": _t("migration_ledger", ("transition_id", "migration_id", "from_state", "to_state", "generation", "source_digest", "target_digest", "status", "error_json", "started_at", "completed_at"), primary_key="transition_id"), "outbox_checkpoints": _t("outbox_checkpoints", ("checkpoint_id", "domain", "last_sequence", "updated_at"), primary_key="checkpoint_id")},
...
for _table_name, _columns in _CONTENT_STRICT.items():
    if _table_name in _TABLES["content"]:
...
_FORMAL_STRICT: dict[str, dict[str, tuple[str, ...]]] = {
    "codegraph": {
        "affected_queries": ("query_id","scope_id","start_id","depth","result_limit","relation_filter","result_json","result_digest","created_at"), "checkpoints": ("checkpoint_id","scope_id","domain","sequence","digest","updated_at"), "codegraph_edges": ("edge_id","from_node_id","to_node_id","edge_kind","weight","metadata_json","created_at"), "codegraph_nodes": ("node_id","node_kind","stable_key","label","source_ref","metadata_json","active","created_at","updated_at"), "codegraph_schema_meta": ("key","value"), "edges": ("edge_id","scope_id","revision_id","from_id","to_id","relation","weight","active","created_at"), "graph_scopes": ("scope_id","workspace_id","agent_instance_id","project_ref","provider","share_group_id","runtime_role","trusted_context","created_at"), "migration_map": ("map_id","source_db","source_table","source_pk","source_hash","target_id","target_type","status","created_at","updated_at"), "outbox": ("event_id","scope_id","sequence","event_type","aggregate_id","payload_hash","status","attempts","error","created_at","projected_at"), "revisions": ("revision_id","file_id","scope_id","content_hash","source_revision","revision_number","created_at"), "schema_meta": ("domain","version","marker","updated_at"), "source_files": ("file_id","scope_id","source_id","path","content_hash","source_revision","language","revision_id","active","created_at","updated_at"), "source_tombstones": ("tombstone_id","file_id","scope_id","reason","revision_id","created_at"), "symbols": ("symbol_id","file_id","scope_id","revision_id","name","kind","signature","symbol_hash","line_start","line_end","active","created_at"), "unknown_ledger": ("ledger_id","source_ref","code","detail","status","source_hash","created_at"),
    },
    "assets": {
...
_MARKERS: dict[str, tuple[tuple[str, ...], int]] = {
    ...
    "content": (("2", "memoryguard-v2-phase2-content"), 2),
    "knowledge": (("memoryguard-v2-phase5-knowledge", "memoryguard-v2-phase1"), 1),
    "codegraph": (("1", "memoryguard-v2-phase5-codegraph"), 1),
...
def _domain(name: str, relative_path: str, db_name: str) -> DomainSpec:
    ...
    return DomainSpec(name, relative_path, db_name, markers, version, tables, json_columns, _COMMON_REF_KEYS, outbox, unknown, migration, _REFERENCE_RULES.get(name, ()), frozenset(), (1, 2) if name in {"rules", "content"} else (1,))
