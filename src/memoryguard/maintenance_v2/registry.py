...
_FORMAL_STRICT: dict[str, dict[str, tuple[str, ...]]] = {
    "codegraph": {
        "affected_queries": ("query_id","scope_id","start_id","depth","result_limit","relation_filter","provenance_filter","result_json","result_digest","created_at"), "affected_receipts": ("receipt_id","scope_id","start_ids_json","result_json","digest","depth","result_limit","provenance_filter","consumed","created_at","consumed_at"), "checkpoints": ("checkpoint_id","scope_id","domain","sequence","digest","updated_at"), "codegraph_edges": ("edge_id","from_node_id","to_node_id","edge_kind","weight","metadata_json","created_at"), "codegraph_nodes": ("node_id","node_kind","stable_key","label","source_ref","metadata_json","active","created_at","updated_at"), "codegraph_schema_meta": ("key","value"), "edges": ("edge_id","scope_id","revision_id","from_id","to_id","relation","context","provenance","source_location","metadata_json","weight","active","created_at"), "graph_scopes": ("scope_id","workspace_id","agent_instance_id","project_ref","provider","share_group_id","runtime_role","trusted_context","created_at"), "migration_map": ("map_id","source_db","source_table","source_pk","source_hash","target_id","target_type","status","created_at","updated_at"), "outbox": ("event_id","scope_id","sequence","event_type","aggregate_id","payload_hash","status","attempts","error","created_at","projected_at"), "refresh_queue": ("queue_id","scope_id","path","queued_at"), "revisions": ("revision_id","file_id","scope_id","content_hash","source_revision","revision_number","created_at"), "schema_meta": ("domain","version","marker","updated_at"), "source_files": ("file_id","scope_id","source_id","path","content_hash","source_revision","language","source_role","provenance","revision_id","active","created_at","updated_at"), "source_fingerprints": ("fingerprint_id","scope_id","path","mtime_ns","size","content_hash","updated_at"), "source_tombstones": ("tombstone_id","file_id","scope_id","reason","revision_id","created_at"), "symbols": ("symbol_id","file_id","scope_id","revision_id","name","kind","signature","symbol_hash","line_start","line_end","provenance","source_map_json","metadata_json","active","created_at"), "unknown_ledger": ("ledger_id","source_ref","code","detail","status","source_hash","created_at"),
    },
    "assets": {
